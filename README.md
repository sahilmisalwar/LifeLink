...
Option B (If Port 3000 is occupied) If you receive an error that something is running on port 3000, start the application on port 3001 using:
...
Connect your GitHub repository to a Vercel project
Set the Root Directory to SmartHelmet/Software
Set the Build Command to npm run build and Output Directory to build
Add environment variables (REACT_APP_SUPABASE_URL, REACT_APP_SUPABASE_ANON_KEY) in the Vercel project settings under Settings → Environment Variables
Every push to the connected branch triggers an automatic production build and deploy
